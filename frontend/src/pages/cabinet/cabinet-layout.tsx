...
function AnalyticsScripts() {
  ...
  useEffect(() => {
    ...
    if (config.googleAnalyticsId?.trim()) {
      ...
      script.onerror = () => { /* Google Analytics 不可达时静默失败 */ };
    ...
    if (config.yandexMetrikaId?.trim()) {
      ...
      script.id = "ym-script";
      script.async = true;
      script.textContent = `(function(m,e,t,r,i,k,a){m[i]=m[i]||function(){(m[i].a=m[i].a||[]).push(arguments)};m[i].l=1*new Date();for(var j=0;j<document.scripts.length;j++){if(document.scripts[j].src===r)return;}k=e.createElement(t),a=e.getElementsByTagName(t)[0],k.async=1,k.src=r,k.onerror=function(){},a.parentNode.insertBefore(k,a)})(window, document, "script", "https://mc.yandex.ru/metrika/tag.js", "ym");ym(${ymId}, "init", {clickmap:true,trackLinks:true,accurateTrackBounce:true,webvisor:true});`;
      document.head.appendChild(script);
    }
  }, [config]);
  return null;
...
function CabinetShell() {
  ...
  return (
    <div className="min-h-svh flex flex-col bg-transparent">
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-4 py-6 overflow-x-clip transition-all duration-300">
        <Outlet />
      </main>
    </div>
  );
